...
function inferQuestions(server: McpServer, plan: Extract<InstallPlan, { kind: 'local-config' | 'remote-config' }>): InstallQuestion[] {
...
function applyAnswers(
  plan: Extract<InstallPlan, { kind: 'local-config' | 'remote-config' }>,
  answers: Record<string, string>,
  requirements: InstallRequirement[]
): ClientMcpConfig {
...
function dedupeQuestions(questions: InstallQuestion[]): InstallQuestion[] {
...
function toPackageArgumentQuestion(arg: PackageArgument, index: number): InstallQuestion {
  return {
...
function toRequirementQuestion(requirement: InstallRequirement): InstallQuestion {
  const sourceText =
...
function compactPrompt(requirement: InstallRequirement): string {
