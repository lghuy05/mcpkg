...
import { ClientMcpConfig, VerificationResult } from '../types/mcp.js';

const DEFAULT_VERIFY_TIMEOUT_MS = 30000;

export async function verifyConfig(config: ClientMcpConfig): Promise<VerificationResult> {
...
async function verifyLocalConfig(
  command: string,
  args: string[],
...
function classifyProcessExit(stderr: string, stdout: string) {
  ...
  if (
    ...
    return 'invalid_launcher' as const;
  }

  return 'process_exited' as const;
}

function compactDetails(...chunks: string[]): string[] | undefined {
...
function compactChunk(chunk: string): string | undefined {
