...
export async function findServerWithAgent(query) {
    ...
    try {
        ...
        const selected = candidates.find((entry) => entry.server.id === choice.serverId) ??
...
async function findHeuristically(query) {
...
async function decidePrimaryQuery(query, apiKey) {
    try {
        ...
        const primaryQuery = typeof parsed.primaryQuery === 'string' && parsed.primaryQuery.trim()
            ? parsed.primaryQuery.trim()
            : heuristicDecision(query)[0];
        const fallbackQueries = Array.isArray(parsed.fallbackQueries)
            ? parsed.fallbackQueries.filter((value) => typeof value === 'string')
            : [];
        return {
            primaryQuery,
            fallbackQueries,
...
async function chooseBestCandidate(query, entries, apiKey) {
    const model = process.env.GEMINI_MODEL || 'gemini-2.5-flash-lite';
    ...
    return {
        ...
        rationale: Array.isArray(parsed.rationale)
            ? parsed.rationale.filter((value) => typeof value === 'string')
            : [],
    };
}
function buildChoicePrompt(query, entries) {
...
function filterCandidatesByExplicitRuntime(query, entries) {
...
function heuristicDecision(query) {
    const normalized = query.toLowerCase();
...
function expandSearchTerms(userQuery, terms) {
...
function formatRankingFailureReason(error) {
