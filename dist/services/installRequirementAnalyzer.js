...
export async function analyzeInstallRequirements(server) {
...
function registryRequirements(server) {
    const requirements = [];
...
async function readmeRequirements(server) {
...
async function loadDocs(server) {
    const docs = [];
...
async function fetchRepositoryReadme(repository) {
...
async function fetchNpmReadme(pkg) {
    try {
        const response = await fetchWithTimeout(`https://registry.npmjs.org/${encodeURIComponent(pkg.identifier)}`);
        if (!response.ok) {
            return null;
        }
        const data = await response.json();
...
function heuristicReadmeRequirements(docs) {
    const requirements = [];
...
function buildRequirementPrompt(server, docs) {
    return [
...
function normalizeAgentRequirements(rawRequirements) {
