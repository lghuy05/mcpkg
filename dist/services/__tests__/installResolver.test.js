...
test('applies env answers into local MCP config', () => {
    ...
    const resolution = resolveInstallPlan(server, plan, { 'env:DATABASE_URL': 'postgresql://user:pass@localhost:5432/db' }, requirements);
...
test('applies remote variables and headers into remote MCP config', () => {
    ...
    const resolution = resolveInstallPlan(remoteServer, plan, {
