...
test('invalid mcpServers shape fails clearly', () => {
    assert.throws(() => parseMcpConfig('{"mcpServers":[]}', 'test.json'), /test\.json\.mcpServers must be an object/);
});
test('invalid server config fails clearly', () => {
    assert.throws(() => parseMcpConfig('{"mcpServers":{"bad":{"command":"npx"}}}', 'test.json'), /test\.json\.mcpServers\["bad"\] is not a valid MCP config/);
});
