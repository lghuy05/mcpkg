...
export function registerInstallCommand(program) {
    program
        ...
        try {
            ...
            if (!verification.ok) {
                const retryDecision = await suggestInstallRetry(selectedEntry.server, resolution.config, verification);
                if (retryDecision.kind === 'fail') {
                    console.log(chalk.yellow(retryDecision.reason));
                    const alternatives = await suggestAlternativeServers(selectedEntry.server, serverName);
                ...
                Object.assign(answers, await promptForInstallQuestions([retryDecision.question]));
                resolution = resolveInstallPlan(selectedEntry.server, plan, answers, requirements);
                if (resolution.kind === 'manual' || !resolution.config) {
...
function hasDuplicate(servers, serverKey, config, options) {
